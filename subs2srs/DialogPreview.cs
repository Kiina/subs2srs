...
namespace subs2srs
{
    ...
    public partial class DialogPreview : Form
    {
        ...
        private void DialogPreview_Load(object sender, EventArgs e)
        {
            ...
            {
                listViewLines.Items[0].Selected = true;
                listViewLines.Select();
            }
        ...
        private bool checkVideoPlayerConfigFile()
        {
            ...
            buttonPreviewVideo.Visible = videoPlayerConfigFileExists;
        ...
        private void updateTextPreview(InfoCombined selectedComb)
        {
            ...
            {
                textBoxSubs1.BackColor = activeColor;
                textBoxSubs2.BackColor = activeColor;
                pictureBoxSubs1.BackColor = activeColor;
                pictureBoxSubs2.BackColor = activeColor;
                panelSubs1.BackColor = activeColor;
                panelSubs2.BackColor = activeColor;
            }
            else
            {
                textBoxSubs1.BackColor = inactiveColor;
                textBoxSubs2.BackColor = inactiveColor;
                pictureBoxSubs1.BackColor = inactiveColor;
                pictureBoxSubs2.BackColor = inactiveColor;
                panelSubs1.BackColor = inactiveColor;
                panelSubs2.BackColor = inactiveColor;
            }

            // Disable subs2 textbox if there aren't any subs2 files
            textBoxSubs2.Enabled = Settings.Instance.Subs[1].Files.Length != 0;
            pictureBoxSubs2.Enabled = Settings.Instance.Subs[1].Files.Length != 0;
            panelSubs2.Enabled = Settings.Instance.Subs[1].Files.Length != 0;

            if (selectedComb.Subs1.Text.EndsWith("png" + ConstantSettings.SrsVobsubFilenameSuffix))
            {
                textBoxSubs1.Visible = false;
                pictureBoxSubs1.Visible = true;
                ...
                try
                {
                    ...
                    {
                        Image image =
                            UtilsSnapshot.getImageFromFile(previewWorkerVars.MediaDir + Path.DirectorySeparatorChar +
                                                           vobsubImage);
            ...
            {
                textBoxSubs1.Visible = true;
                pictureBoxSubs1.Visible = false;
                textBoxSubs1.Text = selectedComb.Subs1.Text;
            }
            ...
            if (Settings.Instance.Subs[1].Files.Length != 0)
            {
                ...
                {
                    textBoxSubs2.Visible = false;
                    pictureBoxSubs2.Visible = true;
                    ...
                    try
                    {
                        ...
                        {
                            Image image =
                                UtilsSnapshot.getImageFromFile(
                                    previewWorkerVars.MediaDir + Path.DirectorySeparatorChar + vobsubImage);
                            vobSubImages.Add(image);
                        }
                ...
                {
                    textBoxSubs2.Visible = true;
                    pictureBoxSubs2.Visible = false;
                    textBoxSubs2.Text = selectedComb.Subs2.Text;
                }
            }
            else
            {
        ...
        private void updateGuiWithInfo(InfoCombined selectedComb)
        {
            ...
            textBoxTimings.Text = formatTimeForDisplay(currentInfoComb.Subs1.StartTime)
                                  + "  -  " + formatTimeForDisplay(currentInfoComb.Subs1.EndTime);
        }


        private void listViewLines_ItemSelectionChanged(object sender, ListViewItemSelectionChangedEventArgs e)
        {
            if (listViewLines.SelectedItems.Count != 1 || !enableListViewItemSelected)
            {
                return;
            }

            updateGuiWithInfo((InfoCombined) listViewLines.SelectedItems[0].Tag);

            // Update the find index
            for (int i = 0; i < listViewLines.Items.Count; i++)
            {
                if ((InfoCombined) listViewLines.Items[i].Tag == (InfoCombined) listViewLines.SelectedItems[0].Tag)
                {
                    lastFindIndex = i;
                }
            }
        ...
        private void bw_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            updateStats();
            populateLinesListView(comboBoxEpisode.SelectedIndex);
        ...
        private void pictureBoxImage_Click(object sender, EventArgs e)
        {
            ...
            string snapshotFile = previewWorkerVars.MediaDir + Path.DirectorySeparatorChar +
                                  ConstantSettings.TempImageFilename;
            ...
            if (audioPreviewNeedsUpdating)
            {
                ...
                if (Settings.Instance.AudioClips.UseAudioFromVideo)
                {
                    if (Settings.Instance.VideoClips.Files.Length > 0)
                    {
                        ...
                        {
                            startTime = UtilsSubs.applyTimePad(selectedComb.Subs1.StartTime,
                                -Settings.Instance.AudioClips.PadStart);
                            endTime = UtilsSubs.applyTimePad(selectedComb.Subs1.EndTime,
        ...
        private void backgroundWorkerAudio_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            buttonPreviewAudio.Enabled = true;
            buttonPreviewAudio.Text = "Preview &Audio";
        ...
        private void buttonPreviewAudio_Click(object sender, EventArgs e)
        {
            ...
            if (!backgroundWorkerAudio.IsBusy)
            {
                selectedEpisodeIndex = comboBoxEpisode.SelectedIndex;

                buttonPreviewAudio.Enabled = false;
                buttonPreviewAudio.Text = "Extracting Audio...";
        ...
        {
            return
                $"{(int) time.TimeOfDay.TotalHours}:{time.TimeOfDay.Minutes:00.}:{time.TimeOfDay.Seconds:00.}.{time.TimeOfDay.Milliseconds:000.}";
        }
        ...
        private void populateLinesListView(int episodeIndex)
        {
            ...
            foreach (InfoCombined comb in previewWorkerVars.CombinedAll[episodeIndex])
            {
                ...
                entryToAdd[2] = formatTimeForDisplay(comb.Subs1.StartTime);
                entryToAdd[3] = formatTimeForDisplay(comb.Subs1.EndTime);

                if (isLineTooLong(comb))
                {
                    entryToAdd[4] =
                        formatTimeForDisplay(UtilsSubs.getDurationTime(comb.Subs1.StartTime, comb.Subs1.EndTime)) +
                        " (Long!)";
                }
                else
                {
            ...
            for (int i = 0; i < listViewLines.Items.Count; i++)
            {
                InfoCombined comb = (InfoCombined) listViewLines.Items[i].Tag;

                if (comb.Active)
                {
                    listViewLines.Items[i].BackColor = activeColor;
                }
                else
                {
                    listViewLines.Items[i].BackColor = inactiveColor;
                }

                if (isLineTooLong(comb))
                {
                    listViewLines.Items[i].ForeColor = longLineWarningColor;
                }
                else
                {
                    listViewLines.Items[i].ForeColor = Color.Black;
                }
            }
        ...
        private void updateStats()
        {
            ...
            linesInCurrentEpisode = previewWorkerVars.CombinedAll[comboBoxEpisode.SelectedIndex].Count;

            foreach (InfoCombined comb in previewWorkerVars.CombinedAll[comboBoxEpisode.SelectedIndex])
            {
                if (comb.Active)
                {
            ...
            labelStatsLinesInCurrentEpisodeNum.Text = linesInCurrentEpisode.ToString();
            labelStatsActiveLinesInCurrentEpisodeNum.Text = activeLinesInCurrentEpisode.ToString();
            labelStatsInactiveLinesInCurrentEpisodeNum.Text = inactiveLinesInCurrentEpisode.ToString();
            labelStatsTotalLinesNum.Text = totalLines.ToString();
            labelStatsTotalActiveLinesNum.Text = totalActiveLines.ToString();
            labelStatsTotalInactiveLinesNum.Text = totalInactiveLines.ToString();
        }
        ...
        private void selectAll()
        {
            ...
            for (int i = 0; i < listViewLines.Items.Count; i++)
            {
                listViewLines.Items[i].Selected = true;
            }

            enableListViewItemSelected = true;

            if (listViewLines.SelectedItems.Count > 0)
            {
                updateGuiWithInfo((InfoCombined) listViewLines.SelectedItems[0].Tag);
            }
        }
        ...
        private void buttonSelectNone_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < listViewLines.Items.Count; i++)
            {
                listViewLines.Items[i].Selected = false;
            }
        ...
        private void buttonInvert_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < listViewLines.Items.Count; i++)
            {
                listViewLines.Items[i].Selected = !listViewLines.Items[i].Selected;
            }
        ...
        {
            if (listViewLines.SelectedItems.Count == 0 || previewWorkerVars == null)
            {
                return;
            }
            ...
            for (int i = 0; i < listViewLines.Items.Count; i++)
            {
                if (listViewLines.Items[i].Selected)
                {
                    previewWorkerVars.CombinedAll[comboBoxEpisode.SelectedIndex][i].Active = false;
        ...
        private void buttonActivate_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < listViewLines.Items.Count; i++)
            {
                if (listViewLines.Items[i].Selected)
                {
                    previewWorkerVars.CombinedAll[comboBoxEpisode.SelectedIndex][i].Active = true;
        ...
        private int getNumberOfLongActiveLines(List<List<InfoCombined>> combinedAll)
        {
            ...
            foreach (List<InfoCombined> combList in combinedAll)
            {
                ...
                {
                    if (comb.Active && isLineTooLong(comb))
                    {
                        longLineCount++;
                    }
                }
        ...
        {
            int activeLongLineCount = getNumberOfLongActiveLines(previewWorkerVars.CombinedAll);

            if (activeLongLineCount > 0)
            {
                bool cont = UtilsMsg.showConfirm(
                    $"There are {activeLongLineCount} active lines that have a long duration (> {ConstantSettings.LongClipWarningSeconds} seconds).\r\n\r\n" +
        ...
        private void textBoxSubs1_TextChanged(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < listViewLines.Items.Count; i++)
            {
                if (listViewLines.Items[i].Tag == currentInfoComb
                    && ((InfoCombined) listViewLines.Items[i].Tag).Subs1.Text != textBoxSubs1.Text)
                {
                    currentInfoComb.Subs1.Text = textBoxSubs1.Text;
                    ((InfoCombined) listViewLines.Items[i].Tag).Subs1.Text = textBoxSubs1.Text;
                    listViewLines.Items[i].Text = textBoxSubs1.Text;
                    changesHaveBeenMade = true;
                    break;
                }
            }
        ...
        private void textBoxSubs2_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                if (listViewLines.Items[i].Tag == currentInfoComb
                    && ((InfoCombined) listViewLines.Items[i].Tag).Subs2.Text != textBoxSubs2.Text)
                {
                    currentInfoComb.Subs2.Text = textBoxSubs2.Text;
                    ((InfoCombined) listViewLines.Items[i].Tag).Subs2.Text = textBoxSubs2.Text;
                    listViewLines.Items[i].SubItems[1].Text = textBoxSubs2.Text;
                    changesHaveBeenMade = true;
                    break;
                }
            }
        ...
        private void findNext(string searchText)
        {
            ...
            if (searchText.Length == 0 || listViewLines.Items.Count == 0)
            {
                return;
            }

            if (lastFindIndex > listViewLines.Items.Count)
            {
                lastFindIndex = 0;
            }
            ...
            for (int i = lastFindIndex; i < listViewLines.Items.Count; i++)
            {
                listViewLines.Items[i].Selected = false;
            }

            for (int i = Math.Min(lastFindIndex + 1, listViewLines.Items.Count); i < listViewLines.Items.Count; i++)
            {
                if (searchActive)
                {
                    if (((InfoCombined) listViewLines.Items[i].Tag).Active &&
                        (((InfoCombined) listViewLines.Items[i].Tag).Subs1.Text.ToLower().Contains(searchText)
                         || ((InfoCombined) listViewLines.Items[i].Tag).Subs2.Text.ToLower().Contains(searchText) ||
                         searchText == ""))
                    {
                        found = true;
                    }
                ...
                {
                    if (((InfoCombined) listViewLines.Items[i].Tag).Subs1.Text.ToLower().Contains(searchText)
                        || ((InfoCombined) listViewLines.Items[i].Tag).Subs2.Text.ToLower().Contains(searchText))
                    {
                        found = true;
                    }
            ...
            if (!found)
            {
                for (int i = 0; i <= lastFindIndex; i++)
                {
                    if (searchActive)
                    {
                        if (((InfoCombined) listViewLines.Items[i].Tag).Active &&
                            (((InfoCombined) listViewLines.Items[i].Tag).Subs1.Text.ToLower().Contains(searchText)
                             || ((InfoCombined) listViewLines.Items[i].Tag).Subs2.Text.ToLower()
