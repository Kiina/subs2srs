...
namespace subs2srs
{
    ...
    public partial class DialogAdvancedSubtitleOptions : Form
    {
        ...
        {
            set => subs1FilePattern = value;
        }

        public string Subs2FilePattern
        {
            set => subs2FilePattern = value;
        }

        public string Subs1Encoding
        {
            set => subs1Encoding = value;
        }

        public string Subs2Encoding
        {
            set => subs2Encoding = value;
        }
        ...
        private void buttonActorCheck_Click(object sender, EventArgs e)
        {
                ...
                else
                {
                    ...
                    if (subs1Files.Length > 0)
                    {
                        foreach (string f in subs1Files)
                        {
                            ...
                            {
                                UtilsMsg.showErrMsg(
                                    "Can't check - Incorrect subtitle format found in Subs1 (only .ass/.ssa allowed).");
                                return;
                            }
                        }
                    }
                ...
                else
                {
                    ...
                    if (subs2Files.Length > 0)
                    {
                        foreach (string f in subs2Files)
                        {
                            ...
                            {
                                UtilsMsg.showErrMsg(
                                    "Can't check - Incorrect subtitle format found in Subs2 (only .ass/.ssa allowed).");
                                return;
                            }
                        }
                    }
            ...
            if (radioButtonSubs1Actor.Checked)
            {
                subsNum = 1;
                fileList = subs1Files;
                fileEncoding = Encoding.GetEncoding(InfoEncoding.longToShort(subs1Encoding));
            }
            else
            {
                subsNum = 2;
                fileList = subs2Files;
                fileEncoding = Encoding.GetEncoding(InfoEncoding.longToShort(subs2Encoding));
            }
        ...
        private void updateSettings()
        {
            Settings.Instance.Subs[0].IncludedWords = UtilsCommon.removeExtraSpaces(
                textBoxSubs1IncludedWords.Text.Split(new char[] {';'}, StringSplitOptions.RemoveEmptyEntries));
            Settings.Instance.Subs[0].ExcludedWords = UtilsCommon.removeExtraSpaces(
                textBoxSubs1ExcludedWords.Text.Split(new char[] {';'}, StringSplitOptions.RemoveEmptyEntries));
            ...
            Settings.Instance.Subs[1].IncludedWords = UtilsCommon.removeExtraSpaces(
                textBoxSubs2IncludedWords.Text.Split(new char[] {';'}, StringSplitOptions.RemoveEmptyEntries));
            Settings.Instance.Subs[1].ExcludedWords = UtilsCommon.removeExtraSpaces(
                textBoxSubs2ExcludedWords.Text.Split(new char[] {';'}, StringSplitOptions.RemoveEmptyEntries));
        ...
        private void updateGUI()
        {
            try
            {
                ...
                numericUpDownSubs1ExcludeShorterThanTime.Value =
                    (decimal) Settings.Instance.Subs[0].ExcludeShorterThanTime;
                checkBoxSubs1ExcludeLongerThanTime.Checked = Settings.Instance.Subs[0].ExcludeLongerThanTimeEnabled;
                numericUpDownSubs1ExcludeLongerThanTime.Value =
                    (decimal) Settings.Instance.Subs[0].ExcludeLongerThanTime;
