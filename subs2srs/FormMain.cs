...
namespace subs2srs
{
    public partial class FormMain : Form
    {
        ...
        public FormMain()
        {
            ...
            Text = UtilsAssembly.Title;

            dlgAdvSubs = new DialogAdvancedSubtitleOptions();
            dlgSnapshotDimensionsChooser =
                new DialogVideoDimensionsChooser(DialogVideoDimensionsChooser.VideoDimesionsChoooserType.Snapshot);
            dlgVideoClipDimensionsChooser =
                new DialogVideoDimensionsChooser(DialogVideoDimensionsChooser.VideoDimesionsChoooserType.Video);
        ...
        private void readPreferencesFile()
        {
            ...
            Settings.Instance.Subs[0].IncludedWords = UtilsCommon.removeExtraSpaces(
                ConstantSettings.DefaultIncludeTextSubs1.Split(new char[] {';'},
                    StringSplitOptions.RemoveEmptyEntries));
            Settings.Instance.Subs[1].IncludedWords = UtilsCommon.removeExtraSpaces(
                ConstantSettings.DefaultIncludeTextSubs2.Split(new char[] {';'},
                    StringSplitOptions.RemoveEmptyEntries));
            Settings.Instance.Subs[0].ExcludedWords = UtilsCommon.removeExtraSpaces(
                ConstantSettings.DefaultExcludeTextSubs1.Split(new char[] {';'},
                    StringSplitOptions.RemoveEmptyEntries));
            Settings.Instance.Subs[1].ExcludedWords = UtilsCommon.removeExtraSpaces(
                ConstantSettings.DefaultExcludeTextSubs2.Split(new char[] {';'},
                    StringSplitOptions.RemoveEmptyEntries));
            ...
            Settings.Instance.Subs[0].ExcludeShorterThanTimeEnabled =
                ConstantSettings.DefaultExcludeLinesShorterThanMsSubs1;
            Settings.Instance.Subs[1].ExcludeShorterThanTimeEnabled =
            ...
            Settings.Instance.Subs[0].ExcludeLongerThanTimeEnabled =
                ConstantSettings.DefaultExcludeLinesLongerThanMsSubs1;
            Settings.Instance.Subs[1].ExcludeLongerThanTimeEnabled =
            ...
            ClientSize = new Size(ConstantSettings.MainWindowWidth, ConstantSettings.MainWindowHeight);
        }
        ...
        private void ripAudioFromVideoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dialog.MediaFilePattern = textBoxVideoFile.Text.Trim();
                dialog.Subs1FilePattern = textBoxSubs1File.Text.Trim();
                dialog.Subs2FilePattern = textBoxSubs2File.Text.Trim();
                dialog.OutputDir = textBoxOutputDir.Text.Trim();
                dialog.DeckName = textBoxName.Text.Trim();
                dialog.EpisodeStartNumber = (int) numericUpDownEpisodeStartNumber.Value;
                dialog.UseSubs1Timings = radioButtonTimingSubs1.Checked;
                dialog.UseTimeShift = groupBoxCheckTimeShift.Checked;
                dialog.TimeShiftSubs1 = (int) numericUpDownTimeShiftSubs1.Value;
                dialog.TimeShiftSubs2 = (int) numericUpDownTimeShiftSubs2.Value;
                dialog.SpanEnabled = groupBoxCheckSpan.Checked;
                dialog.SpanStart = maskedTextBoxSpanStart.Text.Trim();
                dialog.SpanEnd = maskedTextBoxSpanEnd.Text.Trim();
                dialog.EncodingSubs1 = comboBoxEncodingSubs1.Text;
                dialog.EncodingSubs2 = comboBoxEncodingSubs2.Text;
                dialog.AudioStreamIndex = comboBoxStreamAudioFromVideo.SelectedIndex;
                dialog.FileBrowserStartDir = lastDirPath;

                int audioBitrate;

                if (!Int32.TryParse(((string) comboBoxAudioClipBitrate.SelectedItem).Trim(), out audioBitrate))
                {
                    audioBitrate = 128;
                }
                ...
                lastDirPath = dialog.FileBrowserStartDir;
            }
            catch (Exception e1)
            {
        ...
        private void buttonSubs1File_Click(object sender, EventArgs e)
        {
            textBoxSubs1File.Text = showFileDialog(textBoxSubs1File.Text.Trim(),
                "All Subtitle Files (*.ass;*.ssa;*.lrc;*.srt;*.trs;*.idx;*.vtt)|*.ass;*.ssa;*.lrc;*.srt;*.trs;*.idx;*.vtt;*.mkv|" +
        ...
        private void linkLabelAdvancedSubtitleOptions_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            try
            {
                dlgAdvSubs.Subs1FilePattern = textBoxSubs1File.Text.Trim();
                dlgAdvSubs.Subs2FilePattern = textBoxSubs2File.Text.Trim();
                dlgAdvSubs.Subs1Encoding = comboBoxEncodingSubs1.Text;
                dlgAdvSubs.Subs2Encoding = comboBoxEncodingSubs2.Text;
        ...
        private void updateSubs(int subsNum)
        {
            ...
            if (subsNum == 1)
            {
                comboVobsub = comboBoxStreamSubs1;
                theLabel = labelVobsubEncoding1;
                comboEncoding = comboBoxEncodingSubs1;
                textbox = textBoxSubs1File;
                file = textBoxSubs1File.Text.Trim();
            }
            else
            {
                comboVobsub = comboBoxStreamSubs2;
                theLabel = labelVobsubEncoding2;
                comboEncoding = comboBoxEncodingSubs2;
            ...
            theLabel.Text = $"Subs{subsNum} Encoding:";
            ...
            if (UtilsSubs.filePatternContainsVobsubs(file))
            {
                ...
                theLabel.Text = $"Subs{subsNum} Stream:";
                ...
                try
                {
                    ...
                    if (firstIdxFile.Length > 0)
                    {
                        ...
                        if (langs.Length > 0)
                        {
                            ...
                            {
                                SubtitleCreator.SUP.StreamsData streamData =
                                    (SubtitleCreator.SUP.StreamsData) sup.LangStreamsData[i];
                                subStreams.Add(new InfoStream(streamData.VobSubId.ToString(), i.ToString(), langs[i],
                                    "sub/idx"));
                            }
                        }
                        else
        ...
        {
            string filePattern = textBoxVideoFile.Text.Trim();
            string[] files = UtilsCommon.getNonHiddenFiles(filePattern);

            comboBoxStreamAudioFromVideo.Enabled = true;
            labelVideoStream.Enabled = true;

            comboBoxStreamAudioFromVideo.Items.Clear();
            ...
            if (files.Length != 0)
            {
                ...
                if (audioStreams.Count > 0)
                {
                    ...
                    {
                        comboBoxStreamAudioFromVideo.Items.Add(stream);
                    }
                }
                else
                {
                    // Show the default stream when the available streams cannot be detected
                    comboBoxStreamAudioFromVideo.Items.Add(new InfoStream());
                }

                comboBoxStreamAudioFromVideo.SelectedIndex = 0;
            }
            else
            {
                comboBoxStreamAudioFromVideo.Enabled = false;
                labelVideoStream.Enabled = false;
            }
        }
        ...
        private void buttonSnapshotDimensionsChoose_Click(object sender, EventArgs e)
        {
            ...
            {
                numericUpDownSnapshotWidth.Value = (decimal) dlgSnapshotDimensionsChooser.NewDimensions.Width;
                numericUpDownSnapshotHeight.Value = (decimal) dlgSnapshotDimensionsChooser.NewDimensions.Height;
            }
        }
        ...
        private void buttonVideoClipDimensionsChoose_Click(object sender, EventArgs e)
        {
            ...
            {
                numericUpDownVideoWidth.Value = (decimal) dlgVideoClipDimensionsChooser.NewDimensions.Width;
                numericUpDownVideoHeight.Value = (decimal) dlgVideoClipDimensionsChooser.NewDimensions.Height;
            }
        }
        ...
        private void loadSettingsFile(string settingsFile)
        {
            ...
            {
                UtilsMsg.showErrMsg(
                    $"Could not load the .{ConstantSettings.SaveExt} file.\nPerhaps it is from an old version of subs2srs.");
            }
        }
        ...
        private void openToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (openDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    loadSettingsFile(openDialog.FileName);
                }
            }
        }
        ...
        private bool updateSettings()
        {
            try
            {
                ...
                Settings.Instance.VideoClips.Files =
                    UtilsCommon.getNonHiddenFiles(Settings.Instance.VideoClips.FilePattern);
                ...
                Settings.Instance.AudioClips.Files =
                    UtilsCommon.getNonHiddenFiles(Settings.Instance.AudioClips.filePattern);
        ...
        private void updateGUI()
        {
            ...
            {
                if (((InfoStream) comboBoxStreamAudioFromVideo.Items[i]).Num ==
            ...
            maskedTextBoxSpanStart.Text =
                UtilsSubs.timeToString(Settings.Instance.SpanStart).Replace(":", "").Remove(0, 1);
        ...
        private void textBoxSubs1File_Validating(object sender, CancelEventArgs e)
        {
            ...
            {
                error =
                    "Please provide a valid subtitle file. \nOnly .srt, .ass, .ssa, .lrc, .trs, .idx and .vtt are allowed.";
                invalidCount++;
            }
        ...
        private void textBoxVideoFile_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (radioButtonExtractAudioFromVideo.Checked && groupBoxCheckGenerateAudioClips.Checked &&
                filePattern == "")
            {
