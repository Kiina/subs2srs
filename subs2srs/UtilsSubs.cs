...
namespace subs2srs
{
    ...
    class UtilsSubs
    {
        ...
        public static DateTime getMidpointTime(DateTime startTime, DateTime endTime)
        {
            ...
            midTime = midTime.AddMilliseconds(midTimeSpan.TotalMilliseconds * 0.5 +
        ...
        public static DateTime applyTimePad(DateTime time, int pad)
        {
            ...
            paddedTime = time.TimeOfDay.TotalMilliseconds + pad >= 0 ? time.AddMilliseconds(pad) : new DateTime();
        ...
        {
            string ext = file.Substring(file.LastIndexOf(".")).ToLower();
            bool isSuppported = ext == ".srt" || ext == ".ass" || ext == ".ssa" || ext == ".idx" || ext == ".lrc" || ext == ".trs" || ext == ".vtt";
            return isSuppported;
        }
        ...
        public static string formatAssColor(Color color, int alpha)
        {
            ...
            outColorStr = $"&H{alpha:X2}{color.B:X2}{color.G:X2}{color.R:X2}";
        ...
        {
            string timeAss =
                $"{(int) time.TimeOfDay.TotalHours}:{(int) time.TimeOfDay.Minutes:00.}:{(int) time.TimeOfDay.Seconds:00.}.{(int) (time.TimeOfDay.Milliseconds / 10):00.}";
