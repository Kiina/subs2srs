...
namespace subs2srs
{
    ...
    class SubsParserLyrics : SubsParser
    {
        ...
        public override List<InfoLine> parse()
        {
            ...
            lineInfos[lineInfos.Count - 1].EndTime =
                new DateTime().AddSeconds(lineInfos[lineInfos.Count - 1].StartTime.TimeOfDay.TotalSeconds + 10);
            ...
            for (int lineIdx = lineInfos.Count - 1; lineIdx >= 0; lineIdx--)
            {
                ...
                if (infoLineCur.Text.Trim().Length == 0
                    || infoLineCur.StartTime.TimeOfDay.TotalMilliseconds ==
