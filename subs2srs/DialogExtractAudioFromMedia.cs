...
namespace subs2srs
{
    ...
    public partial class DialogExtractAudioFromMedia : Form
    {
        ...
        {
            set => textBoxMediaFile.Text = value;
        }
        ...
        {
            set => groupBoxCheckTimeShift.Checked = value;
        }
        ...
        public int Bitrate
        {
            set
            {
                ...
                {
                    if (Int32.Parse((string) comboBoxBitrate.Items[i]) == value)
                    {
        ...
        {
            set => groupBoxCheckSpan.Checked = value;
        }

        public string SpanStart
        {
            set => maskedTextBoxSpanStart.Text = value;
        }

        public string SpanEnd
        {
            set => maskedTextBoxSpanEnd.Text = value;
        }
        ...
        {
            set => comboBoxStreamAudioFromVideo.SelectedIndex = value;
        }
        ...
        private void DialogAudioRipper_Load(object sender, EventArgs e)
        {
            ...
            ActiveControl = textBoxMediaFile;
        ...
        private void updateSettings()
        {
            ...
            useSpan = groupBoxCheckSpan.Checked;
        ...
        private void textBoxMediaFile_TextChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                labelAudioStream.Enabled = false;
            }
        }
        ...
        private void buttonSubs1File_Click(object sender, EventArgs e)
        {
            textBoxSubs1File.Text = showFileDialog(textBoxSubs1File.Text.Trim(),
                "All Subtitle Files (*.ass;*.ssa;*.lrc;*.srt;*.trs;*.vtt;*.mkv)|*.ass;*.ssa;*.lrc;*.srt;*.trs;*.vtt;*.mkv|" +
        ...
        private void bw_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            string endMessage = $"Audio extraction completed in {workerTotalTime.TotalMinutes:0.00} minutes.";
        ...
        private void splitAudio(object sender, DoWorkEventArgs e)
        {
            ...
            foreach (string file in mediaFiles)
            {
                ...
                string progressText = $"Processing audio from media file {episode} of {mediaFiles.Length}";
                ...
                if (!isSingleFile)
                {
                    numClips = (int) Math.Ceiling(mediaDuration.TimeOfDay.TotalMilliseconds /
                                                  (clipLength.TimeOfDay.TotalSeconds * 1000.0));
                }

                for (int clipIdx = 0; clipIdx < numClips; clipIdx++)
                {
                    progressText =
                        $"Splitting segment {clipIdx + 1} of {numClips} from media file {episode} of {mediaFiles.Length}";
                    ...
                    string nameStr = name.createName(ConstantSettings.ExtractMediaAudioFilenameFormat,
                        episode + episodeStartNumber - 1,
                        clipIdx + 1, startTimeName, endTimeName, "", "");


                    string outName = $"{outputDir}{Path.DirectorySeparatorChar}{nameStr}"; // {2}
                    ...
                    if (groupBoxCheckLyrics.Checked)
                    {
                        ...
                        {
                            if (comb.Subs1.StartTime.TimeOfDay.TotalMilliseconds >=
                                startTimeName.TimeOfDay.TotalMilliseconds
        ...
        /// </summary>
        private string formatLyricsPair(InfoCombined comb, UtilsName name, DateTime clipStartTime, int episode,
            int sequenceNum)
        {
            string pair = "";
            ...
            lyricStartTime = lyricStartTime.AddMilliseconds(comb.Subs1.StartTime.TimeOfDay.TotalMilliseconds -
        ...
        private bool isSupportedSubtitleFormat(string file)
        {
            ...
            bool ret = ext == ".srt" || ext == ".ass" || ext == ".ssa" || ext == ".lrc" || ext == ".trs" || ext == ".vtt";
        ...
        private void textBoxSubs1File_Validating(object sender, CancelEventArgs e)
        {
            ...
            if (groupBoxCheckLyrics.Checked)
            {
                ...
                {
                    error = "Please provide a valid subtitle file. \nOnly .srt, .ass, .ssa, .lrcm, .trs and .vtt  are allowed.";
                    invalidCount++;
                }
                ...
                if (error == null)
                {
                    ...
                    {
                        error =
                            "Please provide a valid subtitle file. \nOnly .srt, .ass, .ssa, .lrc, .trs and .vtt are allowed.";
                        invalidCount++;
                    }
                }

                if (error == null && UtilsSubs.getNumSubsFiles(filePattern) !=
                    UtilsCommon.getNonHiddenFiles(textBoxMediaFile.Text.Trim()).Length)
                {
        ...
        private void updateSubs(int subsNum)
        {
            ...
            if (Path.GetExtension(file) == ".mkv")
            {
                ...
                else
                {
                    ...
                    textbox.Text = result == DialogResult.OK ? mkvDlg.ExtractedFile : "";
                }
