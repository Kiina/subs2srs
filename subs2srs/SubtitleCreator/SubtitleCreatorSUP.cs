...
namespace SubtitleCreator
{
    ...
    public class SUP
    {
        ...
        private class SUPdata
        {
            ...
            public Rectangle SubtitleBorders => _SubtitleBorders;
        ...
        public static SUP Instance { get; private set; } = new SUP();
        ...
        public int GetStartTimeMsec(int Index)
        {
            ...
            return (data.PackageStartTime.Hour * 3600 + data.PackageStartTime.Minute * 60 +
                    data.PackageStartTime.Second) * 1000 + data.PackageStartTime.Millisecond;
        }
        ...
        {
            SUPdata data = (SUPdata) SUPList[Index];
            return (data.PackageEndTime.Hour * 3600 + data.PackageEndTime.Minute * 60 + data.PackageEndTime.Second) *
                   1000 + data.PackageEndTime.Millisecond;
        }
        ...
        public static string ReturnStrVobSubID()
        {
        ...
        public bool ConvertSubIdxToSup(string Filename, float ticksPerMSec, ref FileStream fs, string SUPfilename,
            bool RetrievePalette, int StreamIndex)
        {
            ...
            // - timestamps
            // - delays
            string idxfilename = Path.GetDirectoryName(Filename) + Path.DirectorySeparatorChar +
                    ...
                    while (Line != null)
                    {
                        ...
                        if (!Line.StartsWith("#") && Line.Contains("palette:") && RetrievePalette)
                        {
                            ...
                            for (int n = 0; n < 16; n++)
                            {
                                ...
                                subColors[n] = YCCVSColor2Mpeg(Color.FromArgb((int) ((fullcolor & 0xff0000) >> 16),
                                    (int) ((fullcolor & 0x00ff00) >> 8), (int) (fullcolor & 0x0000ff)));
                            }
                        }
                        ...
                        if (ActiveStream == StreamIndex && !Line.StartsWith("#") &&
                            Line.Contains("timestamp: ") && idx_exists)
                        {
                            ...
                            else
                            {
                                ...
                                int PTS_idx = (int) (ticksPerMSec *
                                                     (Convert.ToInt32(PTS_idx_timings[3]) +
                                                      1000 * (Convert.ToInt32(PTS_idx_timings[2]) +
                                                              60 * (Convert.ToInt32(PTS_idx_timings[1]) +
                                                                    60 * Convert.ToInt32(PTS_idx_timings[0])))));
                                ...
                                temp_idxdata.filepos = UInt32.Parse(PTS_idx_filepos[1],
                        ...
                        if (!Line.StartsWith("#") && Line.Contains("delay"))
                        {
                            ...
                            if (Line_splitted[1].Contains("+"))
                            {
                                ...
                                delay_idx = (int) (ticksPerMSec *
                                                   (Convert.ToInt32(Line_splitted[4]) + 1000 *
                                                    (Convert.ToInt32(Line_splitted[3]) +
                                                     60 * (Convert.ToInt32(Line_splitted[2]) +
                                                           60 * Convert.ToInt32(Hours_splitted[1])))));
                            }
                            else if (Line_splitted[1].Contains("-"))
                            {
                                ...
                                delay_idx = (int) (ticksPerMSec *
                                                   (Convert.ToInt32(Line_splitted[4]) + 1000 *
                                                    (Convert.ToInt32(Line_splitted[3]) +
                                                     60 * (Convert.ToInt32(Line_splitted[2]) +
                                                           60 * Convert.ToInt32(Line_splitted[1])))));
            ...
            if (idx_exists)
            {
                foreach (idxdata my_idxdata in List_idxdata)
                {
                    ...
                    if ((i = fs.ReadByte()) == 0 && (i = fs.ReadByte()) == 0 && (i = fs.ReadByte()) == 1 &&
                        (i = fs.ReadByte()) == 0xba)
                    {
                        ...
                        long data_block_size =
                            (b[0] << 8) + b[1]; // Data block size (less than 2028 - 2028 if full block)
        ...
        {
            bool
                PackageEndTimeNotSpecified =
                    true; // Some SUPs don't specify the end display time (so use the new start time to set the previous end time)
            FileStream fs;
                            ...
                            while (fs.Position < NextControl && fs.Position < data.nextSUP)
                            {
                                ...
                                switch (i)
                                {
                                    case 1:
                                        ...
                                        if (TimeOfControl > 0)
                                            data.PackageStartTime =
                                                data.PackageStartTime.AddMilliseconds(
                                                    ((TimeOfControl << 10) + 1023) / ticksPerMSec);
                                        if (PackageEndTimeNotSpecified && GetNoOfSubtitles() > 0)
                                            SetEndTime(GetNoOfSubtitles() - 1,
                                                data.PackageStartTime.AddMilliseconds(-1));
                                    ...
                                    case 2:
                                        ...
                                        // Stop displaying
                                        data.PackageEndTime =
                                            data.PackageEndTime.AddTicks(
                                                data.PackageStartTime.Ticks - data.PackageEndTime.Ticks);
                                        data.PackageEndTime =
                                            data.PackageEndTime.AddMilliseconds(
                                                ((TimeOfControl << 10) + 1023) / ticksPerMSec);
                                        break;
                                    case 3:
                                        // Color mapping
                                    ...
                                    case 4:
                                        ...
                                        fs.Read(b, 0, 2);
                                        if (b[0] == 0 && b[1] == 0)
                                            break; // Added for some sups that seem to have some codes resetting all colors
                                        data.Transparency[3] =
                                            (byte) ((b[0] & 0xF0) >> 4); //(((int) b[0] & 0xF0) == 0);
                                        data.Transparency[2] = (byte) (b[0] & 0x0F); //(((int) b[0] & 0x0F) == 0);
                                        data.Transparency[1] =
                                            (byte) ((b[1] & 0xF0) >> 4); //(((int) b[1] & 0xF0) == 0);
        ...
        public void ReadHDSUP(ref FileStream fs)
        {
            ...
            while ((i = fs.ReadByte()) != -1)
            {
                ...
                if (i == (int) 'S' && (i = fs.ReadByte()) == (int) 'P')
                {
                    ...
                    // Pointer to start control sequence
                    fs.Read(b, 0, 4);
                    data.startControlSeq =
                        refPos + (long) ((int) b[0] << 24 | (int) b[1] << 16 | (int) b[2] << 8 | (int) b[3]);
                    ...
                    while (fs.Position < data.nextSUP)
                    {
                        ...
                        // Read offset to next control sequence
                        fs.Read(b, 0, 4);
                        long tmp = refPos + ((int) b[0] << 24 | (int) b[1] << 16 | (int) b[2] << 8 |
                                             (int) b[
                                                 3]); // Are we dealing with the last control seq (next control equals previous next control)
                        if (NextControl != tmp && tmp > fs.Position)
                            NextControl = tmp;
                        else
                            NextControl = data.nextSUP;
                            ...
                            switch (i)
                            {
                                    ...
                                    if (TimeOfControl > 0)
                                        data.PackageStartTime =
                                            data.PackageStartTime.AddMilliseconds(
                                                ((TimeOfControl << 10) + 1023) / ticksPerMSec);
                                    if (PackageEndTimeNotSpecified && GetNoOfSubtitles() > 0)
                                        SetEndTime(GetNoOfSubtitles() - 1, data.PackageStartTime.AddMilliseconds(-1));
                                    PackageEndTimeNotSpecified = true;
                                    break;
                                case 2:
                                    PackageEndTimeNotSpecified = false;
                                ...
                                case 0x83:
                                    ...
                                    for (int ind = 0; ind < 256; ind++)
                                    {
                                        ...
                                        int RGB =
                                            ((int) Math.Min(Math.Max(Math.Round(1.1644F * Y + 1.596F * Cr), 0),
                                                 255) << 16) +
                                            ((int) Math.Min(
                                                 Math.Max(Math.Round(1.1644F * Y - 0.813F * Cr - 0.391F * Cb), 0),
                                                 255) << 8) +
                    ...
                    if (SetPalette)
                    {
                        ...
                        Grey = Color.FromArgb((White.R + Black.R) / 2, (White.G + Black.G) / 2,
                            (White.B + Black.B) / 2);
        ...
        private void ConvertHDSUP2Bitmap(FileStream fs, SUPdata data)
        {
                ...
                fs.Position > fs.Length - 9096)
            {
                ...
                SDbmpSUP = (Bitmap) ImageResize(_HDbmpSUP, VideoSizeX, Instance.VideoSizeY,
                    System.Drawing.Drawing2D.InterpolationMode.NearestNeighbor);

                // Second convert it to 4 bit/indexed
                BitmapData SD32data = SDbmpSUP.LockBits(new Rectangle(0, 0, SDbmpSUP.Width, SDbmpSUP.Height),
                    ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
                BitmapData SD4data = _bmpSUP.LockBits(new Rectangle(0, 0, _bmpSUP.Width, _bmpSUP.Height),
                ...
                IntPtr ReadScan0 = SD32data.Scan0;
                IntPtr WriteScan0 = SD4data.Scan0;
        ...
        Image ImageResize(Image img, int destWidth, int destHeight,
            System.Drawing.Drawing2D.InterpolationMode Intermode)
        {
            ...
            gr.DrawImage(img, new Rectangle(0, 0, destWidth, destHeight),
                ...
                else
                {
                    for (int i = 0; i < 4; i++)
                    {
                        ...
                        if (data.Transparency[i] == TRANSPARENT)
                        {
                            ...
                            // Set the background color here
                            //pal.Entries[i] = Color.FromArgb(0, mySettings.Colors[Index].R, mySettings.Colors[Index].G, mySettings.Colors[Index].B);
                            pal.Entries[i] = Color.FromArgb(0, subColors[Index].R, subColors[Index].G,
                                subColors[Index].B);
                        }
                        else
                        {
                            ...
                            // Set the foreground colors here
                            //pal.Entries[i] = Color.FromArgb(255, mySettings.Colors[Index].R, mySettings.Colors[Index].G, mySettings.Colors[Index].B);
                            pal.Entries[i] = Color.FromArgb(255, subColors[Index].R, subColors[Index].G,
                                subColors[Index].B);
                        }
                    }
                }
        ...
        private void ChangeColCon(IntPtr Scan0, int stride, SUPdata data, int Xmax)
        {
            ...
            for (int k = 0; k < nPxCTLI; k++)
            {
                ...
                int startColumn = Math.Max(0,
                    (int) ((data.ChgColCon[ptr++] << 8) | data.ChgColCon[ptr++]) - data.BitmapPos.X);
